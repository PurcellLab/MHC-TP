...
def test_run_search_writes_csv(tmp_path):
    ...
    ref = pd.DataFrame({
        "allotype": ["A*02:01"], "formatted": ["A0201"], "mhc_class": ["I"],
        "locus": ["A"], "n_positions": [1], "matrix": [ramp.tolist()],
        "source": ["test"],
    })
    ...
    out = tmp_path / "out"
    run_search(str(tmp_path / "gibbs"), str(ref_path), "human", str(out), threshold=0.5, top_n=3, make_html=False)
...
def test_run_search_writes_html(tmp_path):
    ...
    out = tmp_path / "out"
    run_search(str(tmp_path / "gibbs"), str(tmp_path / "human.parquet"), "human", str(out), threshold=0.5, top_n=3)
