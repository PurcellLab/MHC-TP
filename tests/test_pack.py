...
def test_classify():
    a = classify_allele("HLA-A02:352")
    assert (
        a.mhc_class == "I"
        and a.locus == "A"
        and a.species == "human"
        and a.formatted == "HLAA02352"
    )
    assert (
        classify_allele("H-2-Db").locus == "D"
        and classify_allele("H-2-Db").mhc_class == "I"
    )
    assert (
        classify_allele("H-2-IAb").locus == "IA"
        and classify_allele("H-2-IAb").mhc_class == "II"
    )
    assert (
        classify_allele("DRB1_0101").mhc_class == "II"
        and classify_allele("DRB1_0101").locus == "DR"
        and classify_allele("DRB1_0101").species == "human"
    )
...
def test_build_pack_parquet(tmp_path):
    ...
    n = build_pack_parquet(
        tmp_path, "I", "human", tmp_path / "h.parquet", "NetMHCpan-4.2"
    )
    assert n == 1
...
def test_build_species_reference(tmp_path):
    c1 = tmp_path / "c1" / "all_logos"
    (c1 / "mhc_names").mkdir(parents=True)
    (c1 / "score_mat_el").mkdir()
    (c1 / "mhc_names" / "P_names.txt").write_text("HLA-A02:01 P\n")
    (c1 / "score_mat_el" / "P.txt").write_text(_HDR + _score_row("0.5"))
    c2 = tmp_path / "c2" / "all_logos"
    (c2 / "log_odds").mkdir(parents=True)
    ...
    n_i, n_ii = build_species_reference(
        "human", tmp_path / "c1", tmp_path / "c2", tmp_path / "human.parquet"
    )
