...
def _ref_df():
    return pd.DataFrame({
        "allotype": ["H2-Kb"], "formatted": ["H2Kb"], "mhc_class": ["I"],
        "locus": ["K"], "n_positions": [9],
        "matrix": [np.random.default_rng(1).random(9 * 20).astype(np.float32).tolist()],
...
def test_render_report_writes_standalone_html(tmp_path):
    ...
    gibbs = {"gibbs.1of1.mat": np.random.default_rng(2).random((9, 20)).astype(np.float32)}
    out = tmp_path / "out"
