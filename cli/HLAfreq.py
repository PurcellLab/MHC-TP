...
*** Important: *** The allele frequencies are not guaranteed to sum to 1.

##### -------------------------------------- ###########
                Modified by Sanjay SG Krishna,
                Purcell/Li Lab, BDI, 
                Monash University, 2025.
###### -------------------------------------- ###########
More information on the methods used to combine allele frequencies
can be found in the https://github.com/BarinthusBio/HLAfreq repository.
This is modified from the original code to work to integrate with the MHC-TP project.
Thanks to the original authors for their work on this code.
"""
...
def get_list(search_by: str = "country", timeout: int = 20, format_output: str = "list") -> Union[List[str], pd.DataFrame, Dict[str, List[str]]]:
    """
    Get a list of countries, ethnicities, or regions from the allelefrequencies.net website.
    
    All countries and ethnicities are listed at:
    http://www.allelefrequencies.net/hla6006a.asp
    
    Args:
        search_by (str): Type of data to retrieve. Options:
            - "country": Get list of available countries
            - "ethnic": Get list of available ethnicities  
            - "region": Get list of available regions
            - "all": Get all available options
        timeout (int): Request timeout in seconds. Defaults to 20.
        format_output (str): Output format. Options:
            - "list": Return as simple list(s)
            - "dataframe": Return as pandas DataFrame
            - "dict": Return as dictionary
    
    Returns:
        Union[List[str], pd.DataFrame, Dict[str, List[str]]]: 
            Available options based on search_by parameter and format_output
    ...
    Example:
        >>> countries = get_list("country")
        >>> ethnicities = get_list("ethnic", format_output="dataframe")
        >>> all_data = get_list("all", format_output="dict")
    """
    
    # Validate search_by parameter
    valid_options = ["country", "ethnic", "region", "all"]
    if search_by not in valid_options:
        raise ValueError(f"search_by must be one of {valid_options}, got '{search_by}'")
    
    # Validate format_output parameter
    valid_formats = ["list", "dataframe", "dict"]
    if format_output not in valid_formats:
        raise ValueError(f"format_output must be one of {valid_formats}, got '{format_output}'")
    
    base_url = "http://www.allelefrequencies.net/hla6006a.asp"
    
    try:
        CONSOLE.log("Fetching data from allelefrequencies.net...")
        ...
        # Dictionary to store all options
        all_options = {}
        
        # Find the form containing the select options
        form = soup.find('form', {'name': 'form1'})
        if not form:
            raise Exception("Could not find the main form on the webpage")
        
        # Extract countries
        if search_by in ["country", "all"]:
            country_select = form.find('select', {'name': 'hla_country'})
            if country_select:
                countries = []
                ...
                all_options['country'] = sorted(countries)
                CONSOLE.log(f"Found {len(countries)} countries")
        
        # Extract ethnicities
        if search_by in ["ethnic", "all"]:
            ethnic_select = form.find('select', {'name': 'hla_ethnic'})
            if ethnic_select:
                ethnicities = []
                for option in ethnic_select.find_all('option'):
                    value = option.get('value', '').strip()
                    if value and value != '':  # Skip empty options
                        ethnicities.append(value)
                all_options['ethnic'] = sorted(ethnicities)
                CONSOLE.log(f"Found {len(ethnicities)} ethnicities")
        
        # Extract regions
        if search_by in ["region", "all"]:
            region_select = form.find('select', {'name': 'hla_region'})
            if region_select:
                regions = []
                ...
                all_options['region'] = sorted(regions)
                CONSOLE.log(f"Found {len(regions)} regions")
        
        # Format and return results based on search_by parameter
        if search_by == "all":
            result_data = all_options
        else:
            if search_by not in all_options:
                raise Exception(f"Could not extract {search_by} data from the webpage")
            result_data = all_options[search_by]
        
        # Format output according to format_output parameter
        if format_output == "list":
            if search_by == "all":
                return result_data  # Return dictionary for "all"
            else:
                return result_data  # Return list for specific type
        
        elif format_output == "dataframe":
            if search_by == "all":
                ...
                return pd.DataFrame({search_by: result_data})
        
        elif format_output == "dict":
            if search_by == "all":
                return result_data
            else:
                return {search_by: result_data}
    
    except requests.exceptions.Timeout:
        raise requests.exceptions.RequestException(
            f"Request timeout after {timeout} seconds. Try increasing the timeout value."
        )
    except requests.exceptions.RequestException as e:
        raise requests.exceptions.RequestException(f"Failed to fetch data: {str(e)}")
    except Exception as e:
        raise Exception(f"Failed to parse webpage: {str(e)}")
    
def url_encode_name(name: str) -> str:
    """
    URL encode country/ethnic/region names for use in allelefrequencies.net URLs.
    
    Converts spaces to '+' and special characters to URL encoding:
    - '(' becomes '%28'
    - ')' becomes '%29'
    - Other special characters are also encoded
    
    Args:
        name (str): The original name
        
    Returns:
        str: URL-encoded name ready for use in API calls
        
    Example:
        >>> url_encode_name("Congo (Kinshasa)")
        'Congo+%28Kinshasa%29'
    """
    # Use quote_plus which converts spaces to '+' and other chars to %XX
...
def getAFdata(base_url, timeout=20, format=True, ignoreG=True):
    ...
    if format:
        try:
            ...
            CONSOLE.log("Formatting failed, non-numeric datatypes may remain.")
    return tabs
...
def incomplete_studies(AFtab, llimit=0.95, ulimit=1.1, datasetID="population"):
    ...
    if sum(lmask > 0):
        CONSOLE.log(poplocs[lmask])
        CONSOLE.log(f"{sum(lmask)} studies have total allele frequency < {llimit}")
    umask = poplocs > ulimit
    if sum(umask > 0):
        CONSOLE.log(poplocs[umask])
        CONSOLE.log(f"{sum(umask)} studies have total allele frequency > {ulimit}")
...
def check_resolution(AFtab):
    ...
    if not pass_check:
        CONSOLE.log(resVC)
        CONSOLE.log("Multiple resolutions in AFtab. Fix with decrease_resolution()")
    return pass_check
...
def combineAF(
    ...
    try:
        ...
        CONSOLE.log("column '2n' could not be created")
...
def alleles_unique_in_study(AFtab, datasetID="population"):
    ...
    if not unique:
        CONSOLE.log(f"Non unique alleles in study, is datasetID correct? {datasetID}")
        CONSOLE.log(grouped.size()[grouped.size() > 1])
    return unique
